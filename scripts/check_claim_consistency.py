...
def main():
    parser = argparse.ArgumentParser(
        description="Check active claims for withdrawn diagnostics and missing evidence."
    )
    ...
    try:
        ...
        for path in args.active_result:
            errors.extend(
                f"{path}: withdrawn diagnostic at {x}" for x in withdrawn_paths(read_json(path))
            )
