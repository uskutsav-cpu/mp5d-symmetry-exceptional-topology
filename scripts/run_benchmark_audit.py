...
from mp5d_science.certification import krawczyk_polynomial, radial_finite_polynomial
...
def finite_certificate(size=12):
    ...
    with mp.workdps(65):
        ...
        root = mp.findroot(
            lambda z: mp.polyval(coeff, z),
            (0.53 - 0.38j, 0.531 - 0.381j),
            tol=mp.mpf("1e-58"),
            maxsteps=80,
        )
        center = [mp.nstr(root.real, 55), mp.nstr(root.imag, 55)]
    certificate = krawczyk_polynomial(
        exact["coefficients"], center, "1e-30", problem_id=exact["problem_id"]
    )
...
def main():
    ...
    if not 0 <= args.max_overtone <= 4:
        parser.error("published manifest contains overtones 0..4")
    manifest = read_json(ROOT / "data/regressions/matyjasek_2021.json")
    rows = []
    for record in manifest["records"]:
        if record["overtone"] > args.max_overtone:
            continue
        with mp.workdps(40):
        ...
        for depth in (160, 240, 320):
            try:
                sol = solve_a(
                    params, seed, overtone=record["overtone"], depth=depth, angular_n=16, dps=45
                )
                measurements.append(
                    sol.to_dict() | {"absolute_error_from_published": abs(sol.omega - seed)}
                )
            except Exception as exc:
                measurements.append(
                    {"status": "NUMERICAL_FAILURE", "error": str(exc), "cf_depth": depth}
                )
        minimum = math.degrees(cmath.phase(complex(seed.real, -seed.imag)))
        angle = min(88.5, max(60.0, (minimum + 89.0) / 2))
        rate = (seed * cmath.exp(1j * math.radians(angle))).imag
        length = max(60.0, 16 / rate)
        for n in (160, 240, 320):
            try:
                sol = solve_c(
                    params, seed, radial_n=n, angular_n=16, length=length, angle_deg=angle
                )
                measurements.append(
                    sol.to_dict() | {"absolute_error_from_published": abs(sol.omega - seed)}
                )
                ...
                measurements.append(
                    {"status": "NUMERICAL_FAILURE", "error": str(exc), "radial_n": n}
                )
        passed = all(
            m.get("converged") and m.get("absolute_error_from_published", math.inf) < 1e-5
            for m in measurements
        )
        rows.append(
            {
                "id": record["id"],
                "normalized_published_frequency": [seed.real, seed.imag],
    ...
    payload = {
        "status": "DIAGNOSTIC_AUDIT",
        "benchmark_source": manifest["source"],
        "benchmarks": rows,
        "symbolic_radial": audit_radial_division(),
    ...
    ready = (
        len(rows) == 15
        and all(r["status"] == "NUMERICAL_BENCHMARK_PASS" for r in rows)
    ...
    atomic_json(
        args.output,
        envelope("benchmark-and-algebra-audit", payload, ROOT, allow_development=args.development),
    )
