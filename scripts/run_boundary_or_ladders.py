...
def main():
    ...
    parser.add_argument(
        "--anchor-file",
        type=Path,
        help="explicit single anchor record; never infer overtone labels",
    )
    ...
    anchor = PairAnchor(
        tuple(float(record[k]) for k in ("s", "delta", "mu")),
        record["m1"],
        record["m2"],
        record["ell"],
        tuple(record["branches"]),
        tuple(record["overtone_labels"]),
        tuple(complex(*z) for z in record["saved_frequencies"]),
    )
    base = Resolution(
        cf_depth=240,
        angular_n=24,
        ...
        precision_dps=45,
        contour_length=40,
        scaling_angle_deg=65,
    )
    try:
        payload = (
            physical_boundary(anchor, base, plan["adaptive_boundary"])
            if args.task == "boundary"
