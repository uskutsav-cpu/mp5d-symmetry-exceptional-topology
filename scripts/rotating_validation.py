...
def table_ii():
    ...
    for r in load("table_ii")["records"]:
        ...
        s = solve_qnm_cf(r["a"], r["b"], r["mu"], r["m1"], r["m2"], ell, 0,
                         initial_frequency=ref * 1.02, depth_schedule=SCH)
        out.append({
            "table": "II", "a": r["a"], "b": r["b"], "mu": r["mu"], "k": r["k"],
            "m1": r["m1"], "m2": r["m2"], "l": ell,
            "omega": [s.omega.real, s.omega.imag],
            "omega_ref": r["omega_ref"], "omega_CFM": r["omega_CFM"],
            "diff_vs_ref": abs(s.omega - ref), "diff_vs_CFM": abs(s.omega - cfm),
            "ode_residual": s.ode_residual, "cf_residual": s.cf_residual,
...
def table_iii():
    ...
    for r in load("table_iii")["records"]:
        ...
        A = solve_qnm_cf(r["a"], r["b"], r["mu"], 1, 1, ell, 0,
                         initial_frequency=mm * 1.02, depth_schedule=SCH)
        B = solve_qnm_hill(r["a"], r["b"], r["mu"], 1, 1, ell, 0,
                           initial_frequency=complex(A.omega), sizes=(120, 180, 240, 300))
        out.append({
            "table": "III", "a": r["a"], "b": r["b"], "mu": r["mu"], "k": r["k"],
            "m1": 1, "m2": 1, "l": ell,
            "omega_solverA": [A.omega.real, A.omega.imag],
            "omega_solverB": [B.omega.real, B.omega.imag],
            "omega_MM": r["omega_MM"], "omega_CFM": r["omega_CFM"],
            "diff_vs_MM": abs(A.omega - mm), "diff_vs_CFM": abs(A.omega - cfm),
...
def equal_spin_checks():
    ...
    for a, b, ell, src in [(0.2, 0.3, 2, "IV"), (0.3, 0.4, 2, "IV"),
                           (0.2, 0.3, 4, "VII"), (0.3, 0.4, 4, "VII")]:
        ref = (IV if src == "IV" else VII)[(a, b)]
        s = solve_qnm_cf(a, b, 0.1, 1, 1, ell, 0,
                         initial_frequency=ref * 1.02, depth_schedule=SCH)
        out["off_diagonal"].append({
            "a": a, "b": b, "l": ell, "source_table": src,
            "omega": [s.omega.real, s.omega.imag],
            "published": [ref.real, ref.imag], "diff": abs(s.omega - ref),
    ...
    for x in (0.1, 0.2, 0.3, 0.4):
        ...
        between = (min(c.real for c in nb) < s.omega.real < max(c.real for c in nb)
                   if len(nb) == 2 else None)
        out["diagonal_anomaly"].append({
            ...
            "verdict": ("our value continues the paper's own off-diagonal trend; "
...
def precision_ladder(cases, ladders):
    ...
    for (a, b, mu, m1, m2, ell, seed, label) in cases:
        ...
        for dps, depth in ladders:
            t0 = time.time()
            s = solve_qnm_mp(a, b, mu, m1, m2, ell, 0, initial_frequency=seed,
                             dps=dps, depth=depth)
            with mp.workdps(dps):
                ...
                rows.append({
                    "dps": dps, "depth": depth, "cf_residual": s.cf_residual,
        ...
        out.append({
            "case": label, "a": a, "b": b, "mu": mu, "m1": m1, "m2": m2, "l": ell,
            "ladder": rows, "stable_digits": stable,
...
def main() -> None:
    ...
    ap.add_argument("--out", type=pathlib.Path,
                    default=ROOT / "results" / "rotating_validation.json")
    ap.add_argument("--precision-out", type=pathlib.Path,
                    default=ROOT / "results" / "precision_validation.json")
    args = ap.parse_args()

    t0 = time.time()
    prov = {"commit": commit(), "env_hash": env_hash(),
            "solver_A": "leaver-cf/2.0", "solver_B": "hill-wynn/1.0",
    ...
    if not args.skip_precision:
        ...
        args.precision_out.write_text(json.dumps(
            {"status": "complete", "provenance": prov,
             "note": ("Recurrence coefficients, angular eigenvalue, reduction, CF and "
                      "root finder ALL run at the stated dps. Horizons are recomputed "
                      "at working precision, never imported from the double path."),
             "cases": pr}, indent=2, default=float) + "\n")
        print(f"wrote {args.precision_out}")
        for c in pr:
            print(f"  {c['case']:28s} stable_digits={c['stable_digits']:.1f} "
