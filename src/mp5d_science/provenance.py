    ...
    for directory in (
        ...
        for path in sorted(base.rglob("*")):
            if path.is_file() and not any(
                p.startswith(".") or p == "__pycache__" for p in path.relative_to(base).parts
            ):
                files[path.relative_to(root).as_posix()] = file_hash(path)
    for name in (
        "requirements.lock",
        "requirements-science.lock",
        "requirements-arb.lock",
        "pyproject.toml",
        "pytest-science.ini",
        "README.md",
    ):
...
def provenance(root: Path, *, allow_development: bool = False) -> dict:
    ...
    return {
        ...
        "source_tree_sha256": snap["sha256"],
...
def envelope(kind: str, payload: dict, root: Path, *, allow_development=False) -> dict:
    if not kind:
        raise ValueError("named result kind required")
    record = {
        "schema_version": SCHEMA_VERSION,
        "kind": kind,
        "provenance": provenance(root, allow_development=allow_development),
        "payload": payload,
    }
...
def check_result(
    ...
    *,
    ...
    if not isinstance(record.get("kind"), str) or not record.get("kind"):
        errors.append("missing result kind")
    for key in ("provenance", "payload"):
        if not isinstance(record.get(key), dict):
            errors.append("missing object " + key)
    ...
    try:
        ...
        errors.append("non-JSON or non-finite result")
    prov = record.get("provenance", {})
    if not isinstance(prov, dict):
        return errors
    env = prov.get("environment", {})
    if not isinstance(env, dict) or env.get("sha256") != digest(
        {k: v for k, v in env.items() if k != "sha256"}
    ):
    ...
    if (
        ...
        or not source_files
        or prov.get("source_tree_sha256") != digest(source_files)
    ):
    ...
    if authoritative:
        if not COMMIT_RE.fullmatch(prov.get("source_commit") or ""):
            errors.append("exact source commit missing")
        if prov.get("dirty_checkout") is not False:
            errors.append("dirty checkout cannot be authoritative")
        if prov.get("authority") != "COMMITTED_SOURCE_RUN":
            errors.append("development result is not authoritative")
        if not expected_commit:
            errors.append("science-freeze commit must be specified externally")
...
def read_json(path: Path) -> dict:
    def invalid_constant(x):
        raise ValueError("non-finite JSON literal " + x)
