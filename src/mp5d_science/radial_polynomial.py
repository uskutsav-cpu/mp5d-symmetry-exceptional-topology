...
def coefficient_formula(
    ...
    W = add(
        scale(
            mul(
                add(power(R, 2), scale(power(t, 2), a * a)),
                add(power(R, 2), scale(power(t, 2), b * b)),
            ),
            omega,
        ),
    ...
    H = add(
        scale(power(t, 2), -imaginary_unit * sigma),
        scale(mul(u, t), -(p - p + 3) / 2),
        scale(u, imaginary_unit * outgoing_k * d),
    )
    J = add(
        power(H, 2),
        mul(u, power(t, 2), derivative(H)),
        scale(mul(H, add(power(t, 2), scale(mul(u, t), -2))), -1),
    )
    Bn = add(
        scale(mul(D, t, R), -2),
        scale(mul(power(R, 4), t), 3 * d),
        scale(mul(power(t, 3), power(R, 2)), d * q),
        scale(power(t, 5), -d * v),
    )
    ...
    numerator = add(
        scale(mul(power(L, 2), power(R, 2), J), d * d),
        mul(L, R, Bn, H),
        mul(power(R, 2), power(W, 2)),
        scale(mul(D, power(R, 6)), -mu * mu),
...
def coefficients(params: RadialParameters, omega, lam, *, dps: int = 50, outgoing_k=None):
    ...
    *,
    ...
    try:
        ...
        A, B, C, _ = coefficient_formula(
            a, b, mu, params.m1, params.m2, omega, lam, p, m, k, acb(0, 1)
        )
        return {
            "A": A,
            "B": B,
            "C": C,
            "bits": bits,
            "scope": "FINITE_COEFFICIENT_FORMULA_ONLY",
