...
WITHDRAWN = frozenset(
    {
        "min_root_separation",
        "min_abs_discriminant",
        "root_separation",
        "a1_over_a2",
        "a1/a2",
        "discriminant_independent",
    }
)
...
def jordan_test(
    ...
    *,
    ...
    chain_residual = float(
        np.linalg.norm(T @ u1 - rhs) / max(rhsnorm, derivative_scale, np.finfo(float).tiny)
    )
    kernel_residual = float(np.linalg.norm(T @ u0) / max(scale, 1.0))
    chain_supported = bool(
        nullity == 1
        and solvability <= relative_tol
        and chain_residual <= relative_tol
        and kernel_residual <= relative_tol
    )
    obstruction = None
    ...
    return {
        ...
        "order_two_obstruction": obstruction,
        "order_two_supported": bool(order_two_supported),
...
def puiseux_test(
    ...
    *,
    ...
    for n in (len(t), max(4, len(t) // 2)):
        ...
        scores = {
            str(p): float(np.sqrt(np.mean((z - p * x - np.mean(z - p * x)) ** 2)))
    ...
    supported = all(
        abs(w["exponent"] - 0.5) <= exponent_tol
        and w["fit_residual"] <= residual_tol
        and w["fixed_exponent_scores"]["0.5"]
        < min(w["fixed_exponent_scores"]["0.0"], w["fixed_exponent_scores"]["1.0"])
        for w in windows
    )
    return {
...
class SolverEvidence:
    ...
    def usable(self, tol: float) -> bool:
        return bool(
            self.solver
            and self.discretization_lineage
            and self.independently_located
            and self.converged
            and np.isfinite(self.omega)
            and np.isfinite(self.residual)
            and 0 <= self.residual <= tol
...
def evaluate_ep2(
    evidence: EP2Evidence,
    *,
    residual_tol: float = 1e-8,
    frequency_tol: float = 1e-7,
    gap_tol: float = 1e-6,
) -> dict:
    ...
    partner_ok = (
        len(partner) == len(evidence.partner_frequencies)
        and len(partner) >= 2
        and len({r.discretization_lineage for r in partner}) >= 2
    )
    if not reasons and partner_ok:
        ...
        if partner_spread <= frequency_tol and gap > max(gap_tol, 10 * uncertainty):
            return {
                "verdict": Verdict.DISTINCT_AT_TESTED_POINT.value,
                "gap": float(gap),
                "scope": "TESTED_POINT_ONLY",
                "continuum_certificate": False,
    ...
    if matrix.get("scope") != "FINITE_ANALYTIC_MATRIX_PENCIL" or not matrix.get(
        "order_two_supported"
    ):
        reasons.append("finite-pencil EP2 Jordan/geometric-multiplicity evidence missing")
    if (
        not evidence.loops_converged
        or evidence.first_loop != [1, 0]
        or evidence.second_loop != [0, 1]
    ):
    ...
    if (
        ...
        or not 1 - 1e-5 <= overlap <= 1 + 1e-12
        or not evidence.eigenvector_trend_verified
    ):
        reasons.append("eigenvector collapse trend missing")
    return {
        "verdict": (Verdict.INCONCLUSIVE if reasons else Verdict.EP2_NUMERICALLY_SUPPORTED).value,
        "reasons": reasons,
        "scope": "FINITE_NUMERICAL_PROBLEM",
        "continuum_certificate": False,
    }
