...
def poschl_teller_spectrum(V0: float = 1.0, n: int = 32):
    if not np.isfinite(V0) or V0 <= 0.25:
    ...
    return {
        "status": "FINITE_TESTBED_SPECTRUM",
        "model": "Poschl-Teller",
        "V0": V0,
        "resolution": n,
        "records": quadratic_spectrum(P0, P1, I),
        "mp5d_solver_F_validated": False,
        "massive_quasiresonant_coverage": False,
    }


def testbed_benchmark(V0=1.0, resolutions=(20, 28, 36), max_overtone=3):
    ...
    for n in resolutions:
        ...
        targets = np.array([np.sqrt(V0 - 0.25) - 1j * (k + 0.5) for k in range(max_overtone + 1)])
    ...
    return {
        "status": "TESTBED_PASS" if all(r["max_error"] < 1e-5 for r in rows) else "INCONCLUSIVE",
        "scope": "EXACTLY_SOLVABLE_TESTBED_ONLY",
        "records": rows,
        "mp5d_solver_F_validated": False,
    }
