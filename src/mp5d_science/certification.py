...
def krawczyk_polynomial(
    coefficients: Sequence[Sequence], center: Sequence, radius, *, problem_id: str
) -> dict:
    ...
    if denominator == 0:
        return {
            "status": "INCONCLUSIVE",
            "reason": "singular point preconditioner",
            "scope": "FINITE_EXACT_POLYNOMIAL",
    ...
    E = [
        [
            Interval.point(int(i == j))
            - sum((R[i][k] * J[k][j] for k in range(2)), Interval.point(0))
            for j in range(2)
        ]
        for i in range(2)
    ]
    ...
    image = [
        Interval.point(corrected[i])
        + sum((E[i][j] * delta[j] for j in range(2)), Interval.point(0))
        for i in range(2)
    ]
    ...
    return {
        "status": "CERTIFIED_UNIQUE_FINITE_ROOT" if passed else "INCONCLUSIVE",
        "problem_id": problem_id,
        "scope": "FINITE_EXACT_POLYNOMIAL",
        "arithmetic": "EXACT_RATIONAL_INTERVALS",
        "polynomial_degree": len(coefficients) - 1,
        "box": [x.to_dict() for x in box],
        "krawczyk_image": [x.to_dict() for x in image],
...
def radial_finite_polynomial(ell: int, size: int) -> dict:
    ...
    def get(p, i):
        return p[i] if 0 <= i < len(p) else s.Integer(0)

    def entry(n, j):
        return s.Poly(
            s.expand(get(A, n - j + 2) * j * (j - 1) + get(B, n - j + 1) * j + get(C, n - j)),
            w,
            domain=s.QQ_I,
        )
    ...
    return {
        "problem_id": f"static-massless-radial-l{ell}-K{size}",
        "coefficients": coefficients,
        "size": size,
        "ell": ell,
        "degree": poly.degree(),
        "scope": "FINITE_RAW_RADIAL_DETERMINANT",
        "continuum_certificate": False,
    }
