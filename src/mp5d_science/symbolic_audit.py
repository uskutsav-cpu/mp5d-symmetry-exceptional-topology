...
def audit_radial_division() -> dict:
    u, p, m, aa, bb, j1, j2, w, k, lam, G, sigma = s.symbols("u p m aa bb j1 j2 w k lam G sigma")
    t, R, d = 1 - u, p - m * u, p - m
    q, v = -(p * p + m * m), p * p * m * m
    L = (2 * p - (p + m) * u) * (p + m - 2 * m * u)
    D = d * d * u * L
    W = (
        (R * R + aa * t * t) * (R * R + bb * t * t) * w
        - j1 * (R * R + bb * t * t) * t * t
        - j2 * (R * R + aa * t * t) * t * t
    )
    ...
    degrees = [
        s.degree(d * d * u * L * L * t * t * R * R, u),
        s.degree(2 * d * d * L * L * R * R * H + L * R * Bn, u),
        quotient.degree(),
    ]
    return {
        "status": "PASS"
        if remainder_identity and horizon_identity and degrees == [9, 8, 7]
        else "FAIL",
        "polynomial_degrees": list(map(int, degrees)),
        "remainder_identity": bool(remainder_identity),
        "horizon_cancellation": bool(horizon_identity),
        "assumptions": [
            "p>m>=0",
            "sigma=W(p)/(2*p*(p^2-m^2))",
            "k^2=w^2-mu^2",
...
def audit_symmetry() -> dict:
    ...
    return {
        "status": "PASS" if first and second else "FAIL",
        "spin_identity": first,
        "equal_spin_spheroidicity_zero": second,
        "scope": "EXACT_FINITE_ALGEBRA",
    }
