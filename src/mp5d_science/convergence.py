...
class Resolution:
    ...
    def __post_init__(self):
        ...
        if (
            not 0 < self.scaling_angle_deg < 90
            or self.contour_length <= 0
            or self.continuation_step <= 0
        ):
...
def run_ladder(
    solve: Callable[[Resolution], Measurement],
    settings: Iterable[Resolution],
    *,
    frequency_tol: float = 1e-7,
    residual_tol: float = 1e-8,
    identity_radius: float = 0.1,
) -> dict:
    ...
    for level in levels:
        try:
            ...
            records.append(
                {"requested_resolution": asdict(level), "status": "FAILED", "error": str(exc)}
            )
