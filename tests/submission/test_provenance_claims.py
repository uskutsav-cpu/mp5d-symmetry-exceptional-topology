...
from mp5d_science.claims import check_claim_registry, validate_freeze, withdrawn_paths
from mp5d_science.provenance import atomic_json, check_result, digest, envelope, read_json
...
def test_altered_artifact_is_detected(tmp_path):
    record = development(tmp_path)
    record["payload"]["status"] = "CERTIFIED"
...
def test_ancestor_is_not_freeze_commit(tmp_path):
    ...
    record["provenance"].update(
        source_commit="a" * 40, dirty_checkout=False, authority="COMMITTED_SOURCE_RUN"
    )
    record["integrity_sha256"] = digest(
        {k: v for k, v in record.items() if k != "integrity_sha256"}
    )
    assert not check_result(record, expected_commit="a" * 40, authoritative=True)
    assert any(
        "exact" in x for x in check_result(record, expected_commit="b" * 40, authoritative=True)
    )
...
@pytest.mark.parametrize(
    "key", ["root_separation", "min_root_separation", "a1_over_a2", "min_abs_discriminant"]
)
def test_withdrawn_active_diagnostic_fails(key):
    assert withdrawn_paths({"active": True, key: 0.2})
    assert not withdrawn_paths({"authority": "WITHDRAWN", key: 0.2})
    assert withdrawn_paths({"authority": "WITHDRAWN", "child": {"active": True, key: 0.2}})
...
def test_freeze_requires_every_artifact_and_active_supported_claim(tmp_path):
    registry = {
        "schema_version": 1,
        "science_ready": True,
        "quasiresonant_scope": "EXCLUDED",
        "claims": [
            {
                "id": "x",
                "active": True,
                "status": "SUPPORTED",
                "scope": "FINITE_ALGEBRA",
