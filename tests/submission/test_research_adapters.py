...
def anchor():
    return PairAnchor((0.42, -0.2, 1.8), 2, 0, 4, ("N2", "N3"), (2, 3), (2.5 - 1.5j, 2.6 - 2.05j))


def test_anchor_uses_decimal_parameter_arithmetic():
    p = parameters(anchor(), (0.42, -0.2, 1.8))
    assert p.a == ".22" or p.a == "0.22"
    assert p.b == "0.62"


@pytest.mark.parametrize("step", [0.1, 0.05, 0.025])
def test_coordinate_routes_use_actual_step(step):
    route = coordinate_route((0, 0, 0), (0.2, 0.1, 0.3), step=step)
    assert route[0] == (0, 0, 0) and route[-1] == pytest.approx((0.2, 0.1, 0.3))
    assert np.max(np.linalg.norm(np.diff(route, axis=0), axis=1)) <= step + 1e-14


def test_routes_change_geometry_not_just_names():
    a = coordinate_route((0, 0, 0), (0.2, 0.1, 0.3), (0, 1, 2))
    b = coordinate_route((0, 0, 0), (0.2, 0.1, 0.3), (2, 1, 0))
    assert a != b
