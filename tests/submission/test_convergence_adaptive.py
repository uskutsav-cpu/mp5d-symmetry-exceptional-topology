...
from mp5d_science.convergence import (
    Measurement,
    Resolution,
    axis_ladders,
    robustness_labels,
...
def measured(r):
    return Measurement(
        (1 + 1 / r.cf_depth**4 - 1j, 2 - 1j), (1e-12, 1e-12), "test", "test", r, True
    )
...
def test_fake_precision_is_rejected():
    def fake(r):
        return replace(measured(r), effective_resolution=BASE)
...
def test_one_failed_rung_cannot_be_hidden():
    def sometimes(r):
        if r.cf_depth == 160:
            raise ArithmeticError("unstable pivot")
        return measured(r)
...
def test_unresolved_pair_is_not_separated():
    def collapsed(r):
        return replace(measured(r), frequencies=(1 - 1j, 1 + 1e-11 - 1j))

    assert (
        run_ladder(collapsed, [replace(BASE, cf_depth=n) for n in (80, 160, 320)])["status"]
        == "INCONCLUSIVE"
    )
...
def test_unknown_axis_fails():
    with pytest.raises(ValueError):
        axis_ladders(BASE, {"fake_axis": [1, 2, 3]})
...
def good(x):
    return GapObservation(
        0.5 + x[0] ** 2 + x[1] ** 2, 1e-12, ("a", "b"), True, ("forward", "reverse")
    )


def test_adaptive_interior_minimum():
    result = refine_minimum(
        good, [0.25, 0.25], [(-1, 1)] * 2, initial_radius=[0.25, 0.25], max_levels=18
    )
...
def test_adaptive_boundary_minimum():
    result = refine_minimum(
        lambda x: good((x[0] - 2, x[1])),
        [0.5, 0],
        [(0, 1), (-1, 1)],
        initial_radius=[0.5, 0.5],
        max_levels=18,
    )
...
def test_adaptive_missing_independent_directions_never_passes():
    result = refine_minimum(
        lambda x: replace(good(x), continuation_directions=("forward",)),
        [0, 0],
        [(-1, 1)] * 2,
        initial_radius=[0.1, 0.1],
    )
...
def test_adaptive_budget_exhaustion_is_failure():
    result = refine_minimum(
        good, [0.1, 0.1], [(-1, 1)] * 2, initial_radius=[0.1, 0.1], max_evaluations=2
    )
    assert result["status"] == "INCONCLUSIVE"
