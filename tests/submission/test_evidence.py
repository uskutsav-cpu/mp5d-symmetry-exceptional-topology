...
def witnesses(omega=0j, lineage="contour"):
    return [
        SolverEvidence("A", "recurrence", omega, 1e-14, 1e-12, True, True),
...
def test_withdrawn_statistic_forbidden(name):
    with pytest.raises(ValueError):
        evaluate_ep2(EP2Evidence([], active_diagnostics=[name]))
...
def test_complete_synthetic_ep2_witness():
    ...
    e = EP2Evidence(
        witnesses(),
        matrix=jordan_test([[0, 1], [0, 0]], -np.eye(2), np.zeros((2, 2))),
        puiseux=puiseux_test(t, 2 * np.sqrt(t)),
        first_loop=[1, 0],
        second_loop=[0, 1],
        loops_converged=True,
        eigenvector_overlap=1.0,
        eigenvector_trend_verified=True,
