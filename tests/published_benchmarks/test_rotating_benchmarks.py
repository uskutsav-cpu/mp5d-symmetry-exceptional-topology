...
def test_table_ii(a, b, mu, k, m1, m2, cfm):
    ...
    s = solve_qnm_cf(a, b, mu, m1, m2, ell, 0, initial_frequency=cfm * 1.02, depth_schedule=SCH)
    assert s.converged
...
def test_table_iii_two_spin(a, b, mu, k, cfm):
    ...
    ell = 2 * k + 2
    s = solve_qnm_cf(a, b, mu, 1, 1, ell, 0, initial_frequency=cfm * 1.02, depth_schedule=SCH)
    assert s.converged
    assert abs(s.omega - cfm) < 2e-4, f"got {s.omega}, paper CFM {cfm}"
...
def test_table_v_vi_exchange_pairs(a, b):
    ...
    P = solve_qnm_cf(a, b, 0.1, 1, 0, 3, 0, initial_frequency=2.1 - 0.35j, depth_schedule=SCH)
    Q = solve_qnm_cf(b, a, 0.1, 0, 1, 3, 0, initial_frequency=2.1 - 0.35j, depth_schedule=SCH)
...
def test_equal_spin_family_off_diagonal(a, b, ell, pub):
    s = solve_qnm_cf(a, b, 0.1, 1, 1, ell, 0, initial_frequency=pub * 1.02, depth_schedule=SCH)
    assert abs(s.omega - pub) < 3e-4, f"got {s.omega}, paper MM {pub}"


@pytest.mark.parametrize(
    "x,published_diagonal",
    [
