...
from mp5d_science.tracking import (Mode, TrackingFailure, TrackingPolicy, assign,
                                   continue_branches, loop_permutation, predict)
...
def test_predictor_rejects_old_off_by_one_coordinates():
    with pytest.raises(ValueError, match="same positive length"):
        predict([[0j], [1+0j]], [0., 1., 2.], 2.)


def test_linear_predictor_is_not_shifted():
    assert predict([[2+3j], [5+3j]], [0., 1.], 2.)[0] == 8+3j


@pytest.mark.parametrize('offset', [0., 1e6, 1e10])
def test_centered_quadratic_predictor(offset):
    assert abs(predict([[0], [1], [4]], [offset, offset+1, offset+2], offset+3)[0]-9) < 1e-12
...
def test_global_assignment_beats_greedy_with_explicit_nonlocal_use():
    ...
    ordered, report = assign(modes([0, 1]), modes([.9, -2]), [0, 1], policy, enforce_motion=False)
    assert [m.omega for m in ordered] == [-2, .9]
...
def test_forward_reverse_and_step_halving(steps):
    ...
    assert abs(forward.modes[-1][0].omega-(-1.9-.5j))<1e-12
...
def test_eigenvectors_preserve_identity_at_real_part_crossing():
    ...
    before=[Mode(-.01+1j,0,np.array([1,0])), Mode(.01-1j,0,np.array([0,1]))]
    after=[Mode(-.01-1j,0,np.array([0,1])),Mode(.01+1j,0,np.array([1,0]))]
    ordered,_=assign(before,after,[.01+1j,-.01-1j],policy)
    assert ordered[0].omega==.01+1j


def test_nonclosed_loop_rejected():
    r=continue_branches(line_solver,[[0],[.1]],modes([-2,2]))
