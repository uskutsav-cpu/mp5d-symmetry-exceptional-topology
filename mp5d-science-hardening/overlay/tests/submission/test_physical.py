...
import numpy as np
import pytest
from mp5d_science.physical import solve_a,solve_c,ContourProblem
from mp5d_science.radial_polynomial import RadialParameters
from mp5d_science.symbolic_audit import audit_radial_division
from mp5d_science.hyperboloidal import chebyshev_lobatto,testbed_benchmark as run_testbed_benchmark
...
def test_corrected_c_static_frequency():
    sol=solve_c(STATIC,.534-.384j,radial_n=180,angular_n=16,length=80)
    assert sol.converged
    assert abs(sol.omega-(.533835574268-.383375368512j))<1e-9

def test_fixed_bordered_residual_is_locally_holomorphic():
    prob=ContourProblem(STATIC,.54-.39j,radial_n=80,angular_n=16,length=80)
    z=.541-.391j;h=1e-5
...
def test_old_svd_residual_is_a_nonnegative_singular_value():
    prob=ContourProblem(STATIC,.54-.39j,radial_n=40,angular_n=16,length=80)
    mat=prob.matrix(.541-.391j)
...
def test_quasiresonant_contour_is_rejected():
    ...
    with pytest.raises(ValueError,match='radiation|suppression'):
        solve_c(params,1.70277948-.00026860j,angle_deg=80)
...
def test_static_cf_cross_inversions_agree():
    sol=solve_a(STATIC,.534-.384j,depth=240,angular_n=16,dps=40)
    assert sol.converged
...
def test_historical_inversion_specific_root_is_not_accepted():
    ...
    sol=solve_a(p,2.5094126155849015-1.5480379310838372j,overtone=2,depth=200,angular_n=24,dps=45)
...
def test_legacy_cf_guard_accepts_the_valid_static_root():
    ...
    result=check_existing_cf_root(STATIC,.533835574268-.383375368512j,depth=240,angular_n=16)
    assert result['valid']
...
def test_legacy_cf_guard_rejects_the_historical_inversion_specific_root():
    from mp5d_science.physical import check_existing_cf_root
    p=RadialParameters('.22','.62','1.8',2,0,4)
    result=check_existing_cf_root(p,2.5094126155849015-1.5480379310838372j,depth=200,angular_n=24,overtone=2)
