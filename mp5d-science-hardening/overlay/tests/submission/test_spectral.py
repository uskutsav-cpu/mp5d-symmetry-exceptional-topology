...
def test_raw_meromorphic_cf_is_not_accepted():
    with pytest.raises(AnalyticityError):count_zeros(lambda z:(z-.1)/(z-.2),0,1)


def test_known_pole_inside_domain_is_rejected():
    with pytest.raises(AnalyticityError):
        AnalyticSpectralFunction(lambda z:(z-.1)/(z-.2),0,1,'claimed','regularized_evans_function',(.2,))
...
@pytest.mark.parametrize('roots',[[],[.2],[.2,-.3],[.1+.2j,-.3+.1j,.4]])
def test_numerical_root_count(roots):
    coefficients=np.poly(roots).tolist() if roots else [0,1]
    if not roots:
        f=AnalyticSpectralFunction(lambda z:1,0,2,'constant entire','explicit_entire_function')
    else:f=polynomial_spectral(coefficients,0,2)
    r=count_zeros(f,0,1)
...
def test_root_on_contour_fails():
    ...
    with pytest.raises(ContourFailure):count_zeros(f,0,1)


def test_outside_analytic_domain_fails():
    f=polynomial_spectral([1,0],0,1)
    with pytest.raises(AnalyticityError):count_zeros(f,.5,.75)


def test_determinant_uses_fixed_scale():
    f=finite_determinant(lambda z:np.array([[z,1],[0,z-.5]]),0,2,
                         'all entries are degree-one polynomials',np.array([2.,3.]))
    assert abs(f(.2)-(.2*(.2-.5))/6)<1e-15
...
def test_svd_projection_is_not_analytic_regression():
    ...
    def bad(z):
        m=np.diag([z,2+0j]);U,s,Vh=np.linalg.svd(m)
        return U[:,-1].conj() @ m @ Vh[-1].conj()
    for z in [.2,.2j,-.2,-.2j]:
        assert abs(bad(z)-.2)<1e-14
    h=1e-5; z=.2+.1j
