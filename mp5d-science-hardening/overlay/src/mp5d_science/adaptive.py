...
class GapObservation:
    ...
    def validate(self, required_directions: int) -> None:
        if (not np.isfinite(self.gap) or self.gap <= 0 or not np.isfinite(self.uncertainty) or
                self.uncertainty < 0 or self.gap <= 10*self.uncertainty):
...
def refine_minimum(evaluate: Callable[[tuple[float, ...]], GapObservation],
                   initial: Sequence[float], bounds: Sequence[tuple[float, float]], *,
                   initial_radius: Sequence[float], parameter_tol: float = 1e-4,
                   value_tol: float = 1e-7, max_levels: int = 15, max_evaluations: int = 10000,
                   required_directions: int = 2) -> dict:
    ...
    if np.any(box[:, 0] >= box[:, 1]) or np.any(x < box[:, 0]) or np.any(x > box[:, 1]) or np.any(radius <= 0):
    ...
    for level in range(max_levels):
        ...
        history.append({"level": level, "point": list(best_point), "gap": best.gap,
                        "uncertainty": best.uncertainty, "radius": radius.tolist(),
                        "movement": movement, "gap_change": change, "complete_stencil": not level_failed})
        if (not level_failed and change is not None and movement <= parameter_tol and
                change <= value_tol and max(radius) <= parameter_tol):
            stable += 1
        else:
            stable = 0
        ...
        if calls >= max_evaluations:
            break
        radius /= 2
    return {"status": status, "point": x.tolist(), "gap": previous, "levels": history,
            "failures": failures, "evaluations": calls, "cached_points": len(cache),
            "scope": "LOCAL_REFINEMENT_ONLY", "global_exclusion_proved": False,
            "boundary_active": [bool(abs(v-lo)<=parameter_tol or abs(v-hi)<=parameter_tol)
