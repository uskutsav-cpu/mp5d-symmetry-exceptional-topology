...
class TrackingPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("residual_tol", "duplicate_atol", "predictor_atol", "predictor_rtol",
                     "assignment_margin", "min_step"):
...
def predict(history: Sequence[Sequence[complex]], times: Sequence[float], target: float) -> np.ndarray:
    ...
    for i, a in enumerate(modes):
        for b in modes[i + 1:]:
            ...
            if abs(a.omega - b.omega) <= floor:
                raise TrackingFailure("duplicate or unresolved roots: coalescence is NOT established")


def _gap(modes: Sequence[Mode]) -> float:
    return min((abs(a.omega - b.omega) for i, a in enumerate(modes)
                for b in modes[i + 1:]), default=float("inf"))


def assign(previous: Sequence[Mode], candidates: Sequence[Mode], predictions: Sequence[complex],
           policy: TrackingPolicy = TrackingPolicy(), *, enforce_motion: bool = True
    ...
    return ordered, {"permutation": cols.tolist(), "predictor_miss": float(np.max(misses)),
                     "max_motion": float(motion), "min_gap": float(separation),
...
def continue_branches(solver: RootSolver, path: Sequence[Sequence[float]], seeds: Sequence[Mode],
                      policy: TrackingPolicy = TrackingPolicy()) -> TrackResult:
    ...
    for vertex in vertices[1:]:
        ...
        while pending:
            ...
            try:
                ...
                retries.append({"target": target.tolist(), "step": step, "level": level,
                                "reason": str(exc), "evaluation": evaluations})
        ...
        if failure is not None:
            break
        completed += 1
    return TrackResult(params, modes, times, retries, evaluations, completed,
