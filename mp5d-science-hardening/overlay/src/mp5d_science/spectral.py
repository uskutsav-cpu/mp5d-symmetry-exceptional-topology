...
class AnalyticSpectralFunction:
    ...
    def __post_init__(self) -> None:
        ...
        if self.representation not in {"polynomial", "finite_analytic_determinant",
                                       "regularized_evans_function", "explicit_entire_function"}:
            raise AnalyticityError("uncontrolled meromorphic or nonanalytic representation")
        if any(not np.isfinite(s) or abs(s-self.center) <= self.radius for s in self.known_singularities):
...
def polynomial_spectral(coefficients: list[complex], center: complex, radius: float) -> AnalyticSpectralFunction:
    ...
    return AnalyticSpectralFunction(lambda z: complex(np.polyval(c, z)), center, radius,
                                    "A finite polynomial is entire.", "polynomial")


def finite_determinant(matrix: Callable[[complex], np.ndarray], center: complex, radius: float,
                       entrywise_analyticity_justification: str,
                       fixed_row_scale: np.ndarray | None = None) -> AnalyticSpectralFunction:
    ...
    scale = np.ones(sample.shape[0]) if fixed_row_scale is None else np.asarray(fixed_row_scale, dtype=float)
    ...
    return AnalyticSpectralFunction(evaluate, center, radius,
                                   entrywise_analyticity_justification, "finite_analytic_determinant")
...
def count_zeros(f: AnalyticSpectralFunction, center: complex, radius: float, *,
                initial_n: int = 64, max_n: int = 16384, integer_tol: float = 1e-7,
                max_phase_step: float = np.pi/3) -> dict:
    ...
    while n <= max_n:
        ...
        resolved = (abs(winding-rounded) <= integer_tol and
                    np.max(abs(increments)) < max_phase_step and rounded >= 0)
        records.append({"n": n, "winding": winding,
                        "max_phase_step": float(np.max(abs(increments))), "resolved": bool(resolved)})
        consecutive = consecutive+1 if resolved and prior == rounded else (1 if resolved else 0)
        if consecutive >= 3:
            return {"count": rounded, "converged": True, "ladder": records,
                    "scope": "NUMERICAL_ARGUMENT_PRINCIPLE", "rigorous": False}
        prior = rounded if resolved else None
        n *= 2
    raise ContourFailure(f"root count did not stabilize: {records}")


def roots_in_disc(f: AnalyticSpectralFunction, center: complex, radius: float, *,
                  initial_n: int = 128, max_roots: int = 8,
                  moment_tol: float = 1e-6) -> tuple[list[complex], dict]:
    ...
    for size in (n, 2*n):
        ...
        for k in range(1, count+1):
            elementary.append(sum((-1)**(j-1)*elementary[k-j]*moments[j]
