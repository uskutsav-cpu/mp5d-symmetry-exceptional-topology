...
from .convergence import Resolution,Measurement,axis_ladders,run_ladder
...
class PairAnchor:
    ...
    def __post_init__(self):
        ...
        if not all(np.isfinite(z) for z in self.frequencies) or abs(self.frequencies[0]-self.frequencies[1])<1e-8:
...
def coordinate_route(start,end,order=(0,1,2),step=.01):
    ...
    current=np.array(start,dtype=float);end=np.array(end,dtype=float)
    ...
    for axis in order:
        ...
        for j in range(1,n+1):
            current=current.copy();current[axis]=beginning+delta*j/n
...
class PairEvaluator:
    def __init__(self,anchor: PairAnchor,resolution: Resolution,agreement_tol=2e-6):
        ...
        self.anchor=anchor;self.resolution=resolution;self.agreement_tol=agreement_tol
        self.calls=[];self.preflight_passed=False;self.anchor_modes=[]

    def pair(self,point,seeds):
        p=parameters(self.anchor,point);r=self.resolution
        primary=[];checks=[]
        for seed,N in zip(seeds,self.anchor.overtones,strict=True):
            A=solve_a(p,seed,overtone=N,depth=r.cf_depth,angular_n=r.angular_n,dps=r.precision_dps)
            C=solve_c(p,seed,radial_n=r.radial_n,angular_n=r.angular_n,
                      length=r.contour_length,angle_deg=r.scaling_angle_deg)
            agreement=abs(A.omega-C.omega)
            self.calls.append({'point':list(map(float,point)),'overtone_label':N,
                'A':A.to_dict(),'C':C.to_dict(),'agreement':agreement})
            if not A.converged or not C.converged or agreement>self.agreement_tol:
                raise ValueError('physical pair lacks same-root agreement and cross-inversion consistency')
            primary.append(A);checks.append(C)
        gap=abs(primary[0].omega-primary[1].omega)
        error=max(abs(a.omega-c.omega) for a,c in zip(primary,checks,strict=True))
        if gap<=max(10*error,1e-7):raise ValueError('pair not resolved above numerical cross-check error')
        return primary,checks

    def preflight(self):
        primary,checks=self.pair(self.anchor.point,self.anchor.frequencies)
        displacement=max(abs(a.omega-w) for a,w in zip(primary,self.anchor.frequencies,strict=True))
        if displacement>self.agreement_tol:
            raise ValueError('independent solver found different roots than the named anchor')
        self.anchor_modes=[Mode(a.omega,a.residual,uncertainty=abs(a.omega-c.omega))
                           for a,c in zip(primary,checks,strict=True)]
        self.preflight_passed=True
        return primary,checks

    def follow(self,path):
        if not self.preflight_passed:raise ValueError('anchor preflight has not passed')
        r=self.resolution
        def solve(point,predictions):
            p=parameters(self.anchor,point)
            modes=[]
            for w,N in zip(predictions,self.anchor.overtones,strict=True):
                out=solve_a(p,w,overtone=N,depth=r.cf_depth,angular_n=r.angular_n,dps=r.precision_dps)
        ...
        if not result.complete:raise ValueError('continuation route failed before its requested endpoint')
        return result

    def observation(self,point):
        if not self.preflight_passed:self.preflight()
        results=[]
        names=[]
        ...
        for axis,order in ((0,(0,1,2)),(1,(2,1,0))):
            ...
            lo,hi=((0.,.42),(-.2,.2))[axis]
            waypoint[axis]+= .005 if waypoint[axis]+.005<=hi else -.005
            if not lo<=waypoint[axis]<=hi:raise ValueError('cannot construct independent continuation detour')
            path=coordinate_route(self.anchor.point,waypoint,order,self.resolution.continuation_step)
            ...
            results.append((a,c));names.append('geometric-detour-axis-'+str(axis))
        first=results[0][0]
        drift=max(abs(results[1][0][i].omega-first[i].omega) for i in range(2))
        error=max(drift,*[abs(a.omega-c.omega) for aa,cc in results for a,c in zip(aa,cc,strict=True)],1e-12)
        if drift>self.agreement_tol:raise ValueError('different continuation directions disagree')
        return GapObservation(abs(first[0].omega-first[1].omega),error,self.anchor.labels,True,tuple(names))
    ...
    for axis,levels in axis_ladders(base,axes).items():
        def evaluate(level):
            ...
            if axis=='continuation_step':
                ...
                neighbor=center.copy();neighbor[0]=max(0.,center[0]-.02)
                ...
                if len(path)<3:raise ValueError('nontrivial in-domain step ladder excursion required')
                tracked=evaluator.follow(path)
                A,C=evaluator.pair(anchor.point,tuple(m.omega for m in tracked.modes[-1]))
            return Measurement(tuple(a.omega for a in A),tuple(max(a.residual,c.residual) for a,c in zip(A,C,strict=True)),
                'A-direct+C-bordered','two-independent-radial-discretizations',level,True,
                tuple(abs(a.omega-c.omega) for a,c in zip(A,C,strict=True)))
        reports[axis]=run_ladder(evaluate,levels,frequency_tol=1e-6)
    return {'status':'PASS' if reports and all(r['status']=='PASS' for r in reports.values()) else 'INCONCLUSIVE',
            'axes':reports,'scope':'POINTWISE_NUMERICAL_LADDERS_NOT_CONTINUUM_CERTIFICATION'}
...
def physical_boundary(anchor: PairAnchor,resolution: Resolution,plan: dict):
    ...
    evaluator.preflight()
    result=refine_minimum(evaluator.observation,anchor.point,[(0,.42),(-.2,.2),(0,1.8)],
        initial_radius=plan['initial_radius'],parameter_tol=plan['parameter_tolerance'],
        value_tol=plan['gap_tolerance'],max_evaluations=plan['max_evaluations'])
