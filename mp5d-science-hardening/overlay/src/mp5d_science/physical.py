...
from .radial_polynomial import (COEFFICIENT_VERSION, RadialParameters, coefficient_formula,
                                evaluate, horizons, mul, power)
...
class PhysicalSolution:
    ...
    def to_dict(self):
        return {"omega": [self.omega.real, self.omega.imag], "residual": self.residual,
                "converged": self.converged, "solver": self.solver, "lineage": self.lineage,
                "settings": self.settings, "omega_decimal": list(self.omega_decimal),
                "angular_decimal": list(self.angular_decimal), "diagnostics": self.diagnostics}
...
def angular_matrix(params: RadialParameters, omega: complex, n: int = 40) -> np.ndarray:
    ...
    zoff = 2*j+alpha+beta
    off = np.sqrt(4*(j+1)*(j+alpha+beta+1)*(j+1+alpha)*(j+1+beta)/
                  ((zoff+1)*(zoff+2)**2*(zoff+3)))
    c2 = (omega*omega-float(params.mu)**2)*(float(params.a)**2-float(params.b)**2)
    ell = 2*index+alpha+beta
    return (np.diag(ell*(ell+2)-c2*(1-diag)/2) +
            np.diag(c2*off/2, 1) + np.diag(c2*off/2, -1))


def angular_double(params: RadialParameters, omega: complex, n: int = 40, homotopy_steps: int = 12) -> complex:
    ...
    base = np.diag([float((2*i+abs(params.m1)+abs(params.m2)) *
                         (2*i+abs(params.m1)+abs(params.m2)+2)) for i in range(n)])
    ...
    if float(params.a)**2 != float(params.b)**2:
        for step in range(1, homotopy_steps+1):
            vals, vecs = linalg.eig(base+(original-base)*step/homotopy_steps,
                                   check_finite=False)
...
def angular_highprec(params: RadialParameters, omega, n: int):
    ...
    for j in range(n):
        ...
        if j < n-1:
            sq = mp.mpf(4*(j+1)*(j+alpha+beta+1)*(j+1+alpha)*(j+1+beta))/((z+1)*(z+2)**2*(z+3))
            off2.append(c2*c2*sq/4)
    def determinant(value):
        old, current = mp.mpc(1), (diag[0]-value)/max(1, abs(params.m1)+abs(params.m2)+1)**2
        previous_scale = mp.mpf(max(1, abs(params.m1)+abs(params.m2)+1)**2)
        for j in range(1, n):
            local_scale = mp.mpf((2*j+alpha+beta+1)**2)
            following = ((diag[j]-value)*current-off2[j-1]*old/previous_scale)/local_scale
    ...
    root = mp.findroot(determinant, (guess, guess+mp.mpf('0.0001')), solver='secant',
                       tol=mp.power(10, -mp.mp.dps+8), maxsteps=40, verify=True)
...
def solve_a(params: RadialParameters, seed: complex | tuple[str, str], *, overtone: int = 0,
            depth: int = 200, angular_n: int = 40, dps: int = 40,
            max_steps: int = 40) -> PhysicalSolution:
    ...
    with mp.workdps(dps):
        ...
        def condition(omega):
            ...
            if k.real < 0:
                k = -k
            A, B, C, _ = coefficient_formula(a, b, mu, params.m1, params.m2, omega, lam, p, m, k, mp.j)
        ...
        root = mp.findroot(condition, (x-mp.mpf('0.00013'), x+mp.mpf('0.00017')),
                           solver='secant', tol=tolerance, maxsteps=max_steps, verify=True)
        ...
        cross = {str(i): abs(recurrence_cf(*lifted, depth, i))
        ...
        return PhysicalSolution(complex(root), float(residual),
                                bool(residual < mp.sqrt(tolerance) and cross_ok),
                                "A-direct-algebra/1.0", "frobenius-recurrence",
                                ...
                                {"function_evaluations": calls, "coefficient_version": COEFFICIENT_VERSION,
                                 "cross_inversion_residuals": {i: mp.nstr(v, dps) for i, v in cross.items()},
                                 "cross_inversion_consistent": cross_ok,
                                 "root_validity": "SUPPORTED_FINITE_CF_ROOT" if cross_ok else
                                                  "INVERSION_SPECIFIC_ROOT_REQUIRES_REJECTION",
...
def physical_ode(params: RadialParameters, omega: complex, lam: complex, r):
    ...
    W = (r2+a*a)*(r2+b*b)*omega-params.m1*a*(r2+b*b)-params.m2*b*(r2+a*a)
    G = a*b*omega-a*params.m2-b*params.m1
    V = (W*W/(r2*r2*delta)-G*G/r2-(a*a+b*b)*omega*omega+
         2*omega*(a*params.m1+b*params.m2)-mu*mu*r2-lam)
...
class ContourProblem:
    ...
    """
    def __init__(self, params: RadialParameters, seed: complex, *, radial_n: int = 160,
                 angular_n: int = 40, length: float = 60, angle_deg: float = 60):
    ...
    def horizon_exponent(self, omega):
        ...
        p, m = self.p, self.m
        Wp = (p*p+a*a)*(p*p+b*b)*omega-self.params.m1*a*(p*p+b*b)-self.params.m2*b*(p*p+a*a)
    ...
    def raw_matrix(self, omega):
        ...
        C0 = A*exponent*(exponent-1)/self.rho**2+B*phase*exponent/self.rho+V*phase**2
    ...
    def offgrid_residual(self, omega, coefficients):
        ...
        terms = np.array([A*second, (2*A*p/rho+B*phase)*first,
...
def solve_c(params: RadialParameters, seed: complex, *, radial_n: int = 160,
            angular_n: int = 40, length: float = 60, angle_deg: float = 60,
            tol: float = 1e-10, max_evaluations: int = 100) -> PhysicalSolution:
    problem = ContourProblem(params, seed, radial_n=radial_n, angular_n=angular_n,
    ...
    return PhysicalSolution(omega, float(residual), converged, "C-bordered-collocation/1.0",
                            "recurrence-free-complex-contour",
                            {'radial_n': radial_n, 'angular_n': angular_n, 'precision_dps': 16,
                             'contour_length': length, 'scaling_angle_deg': angle_deg},
                            (format(omega.real, '.17g'), format(omega.imag, '.17g')),
                            (format(lam.real, '.17g'), format(lam.imag, '.17g')),
                            {'function_evaluations': problem.evaluations, 'optimizer_success': bool(root.success),
...
def check_existing_cf_root(params: RadialParameters, omega: complex, *, depth=200,
                           angular_n=40, overtone=0, dps=45, tolerance=1e-6) -> dict:
    ...
    try:
        with mp.workdps(dps):
            ...
            residuals={str(i):float(abs(recurrence_cf(*ABC,depth,i))) for i in range(min(depth-2,max(4,overtone+2)))}
            valid=all(np.isfinite(x) and x<tolerance for x in residuals.values())
        return {'valid':bool(valid),'cross_inversion_residuals':residuals,'tolerance':tolerance,
        ...
        return {'valid':False,'reason':str(exc),'scope':'FINITE_CF_ROOT_CONSISTENCY_NOT_CONTINUUM_CERTIFICATION'}
